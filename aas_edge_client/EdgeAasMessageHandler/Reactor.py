...
import asyncio
from EdgeAasMessageHandler.EdgeHandler import EdgeHandler, Job
...
class AsyncReactor(metaclass=Singleton):
    ...
    async def handle_job(self, handler: EdgeHandler, job: Job):
    ...
    def register_handler(self, type_: str, handlerClass: Type[EdgeHandler]): 
    ...
    def get_handler(self, type_: str) -> EdgeHandler:
        if type_ is None:
            return None
        if type_ in self.dictHandler:
